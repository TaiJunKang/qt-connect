/**
 * 분리된 991개 플랜의 text + commentary를 Opus로 재생성
 * 원본 OCR 큐티 + 성경 원문 기반, 감리교 교리 적합
 */
...
const CONCURRENCY = 3; // Opus는 느리므로 3개 병렬
...
원래 ${origRef}을 아우르는 큐티가 있었는데, 1장 단위로 분리합니다.
...
위 모범 예시의 문체와 깊이를 참고하여, ${plan.reference}만을 다루는 큐티를 작성해 주세요.
  ...
  return Promise.all(
    items.map(async (item) => {
      try {
        ...
        console.error(`  [FAIL] ${item.plan.date} ${item.plan.reference}: ${e.message.slice(0, 100)}`);
...
async function main() {
  console.log("=== QT 재생성 (Opus) ===\n");

  // 1. 재생성 필요한 플랜 가져오기 (분리된 항목들)
  console.log("1. 재생성 대상 로드...");
  const res = await fetch(
    `${SUPABASE_URL}/rest/v1/qt_plans?select=date,title,reference&order=date.asc`,
    { headers: { apikey: SUPABASE_KEY } }
  );
  const allPlans = await res.json();

  // 원본이 단일장이었던 것(text가 이미 OCR 내용)은 제외
  const needsRegen = allPlans.filter(p => {
    const orig = findOriginalPlan(p.reference);
    if (!orig) return true; // 원본 못 찾으면 재생성
    // 원본이 단일장이면 이미 OCR 내용이 들어있으므로 제외
    const rangeM = orig.reference.match(/(\d+)[~\-](\d+)/);
    const crossM = orig.reference.match(/(\d+):\d+[~\-](\d+)/);
    return rangeM || crossM; // 여러 장 범위에서 분리된 것만
  });

  console.log(`   → 전체 ${allPlans.length}개 중 재생성 대상: ${needsRegen.length}개\n`);

  // 2. 각 항목에 성경 본문 + 원본 플랜 매핑
  ...
  // 3. Opus로 생성
  console.log(`2. Opus로 큐티 생성 (${CONCURRENCY}개 병렬)...`);
  const results = [];
  let done = 0;
  ...
  if (failed.length > 0) {
    console.log("   실패 목록:", failed.map(f => f.date).join(", "));
  }
  ...
  for (let i = 0; i < successful.length; i += chunkSize) {
    ...
    sql.push("COMMIT;");
    const fname = `scripts/regen-${String(fileIdx).padStart(3, "0")}.sql`;
    writeFileSync(fname, sql.join("\n"), "utf8");
    fileIdx++;
  }
  ...
  for (let i = 0; i < fileIdx; i++) {
    const fname = `scripts/regen-${String(i).padStart(3, "0")}.sql`;
    try {
  ...
  console.log(`   → SQL 성공: ${sqlOk}, 실패: ${sqlFail}`);
  console.log("\n=== 완료! ===");
}
